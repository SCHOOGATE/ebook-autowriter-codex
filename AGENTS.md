...
### この章で読者が得る変化
読む前: {読者の現在の状態・認識}
読んだ後: {この章で得られる具体的な変化・新しい認識}

### 各節の具体例・データ（リサーチから配分）
- 1.1: {research.md Layer○の具体的な引用元} — {どう使うか}
- 1.2: {Layer5 読者の悩み#○に直接回答} — {回答の方向性}
- 1.3: {Layer1 YouTube専門家○○の手法を発展} — {独自の切り口}
- 1.4: {具体的な数字・統計: ○○は△△%（出典）}
- 1.5: {Layer4 競合書籍にない空白地帯を埋める内容}

### この章では扱わない内容（重複防止）
- {○○の話題 → 第3章1.2で詳述するため、ここでは触れない}
- {△△の事例 → 第5章で使用するため、ここでは別の事例を使う}

### 読者の悩み対応（Layer 5 マッピング）
- 悩み#3「{具体的な悩み}」→ 節1.2で直接回答
- 悩み#7「{具体的な悩み}」→ 節1.4で因果関係を説明

### フック（次章への引き）
「{前章の結論を踏まえ、次章で何を解決するかを1文で予告}」
...
**推奨表現パターン:**
- 抽象→具体: 「運動は健康に良い」→「朝の15分ウォーキングを始めて2週間、午前中の眠気が消えたという報告は多い（Layer1 YouTube: ○○チャンネル）」
...
**Step A: 自動リトライ（ユーザー操作不要・最大5回）**
  1. `mcp__chrome-devtools__list_pages()` を実行
  2. → 成功 → 通常の生成フローへ進む
  3. → 失敗 → 10秒待機 → 再試行（最大5回）
  ※ 5回すべて自動で試行する。1回失敗しただけで諦めない。

**Step B: ユーザーに操作を依頼（Step Aが全敗した場合）**
  「Chrome DevToolsに接続できません。以下を確認してください:
   1. デスクトップの「Chrome（書籍制作用）」をダブルクリックして起動
   2. ChatGPTにログイン済みであること
   準備ができたら「OK」と入力してください。」

**Step C: ユーザー「OK」後に再度自動リトライ（5回）**
  → 成功 → 通常の生成フローへ進む
  → 全敗 → Step D へ

**Step D: Chrome完全再起動を依頼**
  「接続に失敗しました。Chromeを一度すべて閉じてから
   「Chrome（書籍制作用）」で再起動してください。
   準備ができたら「OK」と入力してください。」
  → Step C に戻る（**回数制限なし・永久ループ**）
