...
**根本原因:** Codex（GPT-4o）が文字数基準をクリアするために、節の本文が終わった後にフィラーテキストを大量挿入している。**フィラーの挿入は文字数に含めない。フィラーを除去した後の本文が字数基準を満たす必要がある。**
...
Step 1: Chrome DevTools 接続確認
  ...
  （ポートは config.toml の chrome-devtools 設定に合わせる。デフォルト9222）
  → PASS: 次へ進む
...
- `~/.codex/config.toml` に `[mcp_servers.chrome-devtools]` が設定済み（ポート9224固定）
- デスクトップの「Chrome book creation」ショートカットでChromeを起動済み（ポート9224）
- ChatGPTにログイン済み
...
**Step B: ユーザーに操作を依頼（Step Aが全敗した場合）**
  「Chrome DevToolsに接続できません。以下を確認してください:
   1. デスクトップの「Chrome book creation」をダブルクリックして起動（ポート9224）
...
**Step D: Chrome完全再起動を依頼**
  「接続に失敗しました。Chromeを一度すべて閉じてから
   「Chrome book creation」で再起動してください（ポート9224）。
   準備ができたら「OK」と入力してください。」
