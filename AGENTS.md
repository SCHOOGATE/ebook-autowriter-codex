...
第1章: {章タイトル}（3,500〜4,500字）【16%】感情役割: Problem（問題の明確化）
  ...
  1.1 共感フック — 読者の日常シーンで問題を描写
  1.2 原因解明 — なぜその問題が起きるのか
  1.3 解決の方向性 — この本でどう解決するかの予告
  1.4 実践例 — 問題を認識できた人の変化
  1.5 即実践 — 今日からできる1ステップ

第2章: {章タイトル}（4,000〜5,000字）【18%】感情役割: Agitation（深掘り・原因解明）
  → 読者が「このままだとまずい」→「変わりたい」と感じる章
  2.1〜2.5（同形式、各節にミニPASCALの要素を配分）
...
### 各章内のミニPASCAL構造（5節の感情設計）

**各章の中でも読者の感情を1サイクル回す。** 全節が同じ構造にならないよう、各節に異なる役割を持たせる:

```
節1: 共感フック — この章のテーマを読者の日常シーンで描写
  → 読者: 「あ、これ自分のことだ」
節2: 原因解明 — なぜそうなるのか、因果関係を説明
  → 読者: 「そういうことだったのか」
節3: 解決策の提示 — 具体的な方法論・フレームワーク
  → 読者: 「なるほど」
節4: 実践例・ケーススタディ — 裏付け＋追体験
  → 読者: 「自分でもできそう」
節5: 即実践アクション — 今日からできる1ステップ
  → 読者: 「やってみよう」→ 次章へ
```

**禁止:** 全5節が「説明→説明→説明→説明→まとめ」の同一パターンになること。各節に上記の異なる役割を必ず持たせる。
...
### 各節の役割＋具体例・データ
- 1.1 共感フック: {ペルソナAの日常シーンで問題描写} — Layer5 悩み#{N}に対応
- 1.2 原因解明: {Layer1 専門家○○の知見} — なぜこの問題が起きるか
- 1.3 解決の方向性: {本書独自のフレームワーク予告} — Layer4 競合にない切り口
- 1.4 実践例: {ペルソナAがこの問題を認識した場面} — Layer2 note記事の事例を発展
- 1.5 即実践: {読者ワーク: 「あなたの○○を振り返ってみてください」}

### 一次情報・ストーリー配分（この章の25%以上）
- パターンA（追体験）: 節1.1と1.4でペルソナAの物語（合計600字以上）
- パターンB（リサーチストーリー化）: 節1.2で「○○氏のクライアントは…」（300字以上）
- パターンC（読者ワーク）: 節1.5末尾に問いかけ（200字）
