...
### 段落ルール（人間が書く本のリズムを再現する）

**1段落 = 2〜4文が基本。** 1文=1段落は箇条書きのように見え、AIっぽくなるため禁止。
段落は「1つの主張とその補足」で構成し、段落間に空行1行を入れる。

```
NG（1文=1段落 — AIっぽい・箇条書き的）:
AI副業を学ぶ前に確認してください。
...
OK（2〜4文で1段落 — 人間の文章のリズム）:
AI副業を学ぶ前に確認してください。使える時間は週に何時間あるのか、初期費用はいくらまでなら出せるのか。この2つが見えるだけで、最初に取るべき行動が変わります。

次に考えるのは、学習に使う時間帯です。朝の30分と夜の30分では集中力が違います。自分が楽に始められる時間帯を1週間だけ観察してみてください。
```

**段落の切れ目:** 話題が変わるとき、視点が変わるとき、具体例に入るとき。
**段落内の文数:** 2文（短い段落）〜4文（長い段落）。5文以上は分割する。
