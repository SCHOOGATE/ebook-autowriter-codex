...
### 追体験ペルソナ（3〜5名）
- ペルソナA: {名前}（{年齢}歳・{職業}）— 第1章〜第4章で登場。第1章で問題提示→第4章で解決
- ペルソナB: {名前}（{年齢}歳・{職業}）— 第2章〜第5章で登場。別の切り口で追体験
- ペルソナC: {名前}（{年齢}歳・{職業}）— 第3章で集中登場。Solution実践の詳細ケース
※ ペルソナは架空だが、Layer5の読者の悩みから具体的に設計する
...
- {節タイトル}: {内容の要約} — {research.md Layer○ / ペルソナ○}
- {節タイトル}: {内容の要約} — {データ出典}
- ...（必要な数だけ）

### 一次情報・ストーリー配分（この章の25%以上）
- パターンA（追体験）: ペルソナの物語（合計600字以上）
...
**パターンA: 読者追体験シナリオ（メイン・各章に必須）**
```
「Aさん（32歳・営業職）は毎朝7時に出社し、夕方には疲労で集中力が切れていました。
ある日、通勤電車の中で○○を試したところ…」
```
- chapter_blueprint.md で設計した3〜5名のペルソナを使う
- 各章に1つ、異なるペルソナの物語を**600字以上**で展開
- 第1章で提示したペルソナAの問題が、第4章で解決される**物語の回収**を入れる
- ペルソナの設定はLayer5（読者の悩み）から具体的に設計する
